...
BEGIN
  ...
  SELECT COUNT(*) INTO existe FROM PERSONA WHERE(W_DNI = PERSONA.DNI);
  IF (existe = 0) THEN
...
W_ESPECIALIDAD    ASIGNATURA.ESPECIALIDAD%TYPE
  ...
  oidp INTEGER;
BEGIN
  SELECT COUNT(*) INTO existe FROM ASIGNATURA WHERE (W_NOMBRE = ASIGNATURA.NOMBRE);
  SELECT OID_P INTO oidp FROM PERSONA WHERE (W_PROFESOR = DNI);
  IF (existe = 0) THEN
    INSERT INTO ASIGNATURA VALUES(SEC_ASIGNATURA.NEXTVAL,W_NOMBRE,oidp, W_MATRICULA, W_ESPECIALIDAD);
    COMMIT WORK;
  END IF;
...
AS
  ...
  ide INTEGER;
BEGIN
  SELECT COUNT(*) INTO existe FROM ESPECIALIDAD WHERE ESPECIALIDAD.NOMBRE = W_NOMBRE;
  IF (existe = 0 AND W_PERSONA <= 2500 AND W_PERSONA >= 1) THEN
  ...
  ELSIF (existe = 0 AND W_PERSONA <= 3000 AND W_PERSONA >=2501) THEN
    INSERT INTO ESPECIALIDAD VALUES(SEC_ESPECIALIDAD.NEXTVAL,W_NOMBRE);
    INSERT INTO TIENE_ASIGNADO VALUES(SEC_TIENE.NEXTVAL, SEC_ESPECIALIDAD.CURRVAL, W_PERSONA);
  ELSIF (existe <> 0 AND W_PERSONA <= 2500 AND W_PERSONA >= 1) THEN
    SELECT OID_E INTO ide FROM ESPECIALIDAD WHERE ESPECIALIDAD.NOMBRE = W_NOMBRE;
    INSERT INTO TIENE VALUES(SEC_TIENE.NEXTVAL, ide, W_PERSONA);
  ELSIF (existe <> 0 AND W_PERSONA <= 3000 AND W_PERSONA >=2501) THEN
    SELECT OID_E INTO ide FROM ESPECIALIDAD WHERE ESPECIALIDAD.NOMBRE = W_NOMBRE;
    INSERT INTO TIENE_ASIGNADO VALUES(SEC_TIENE.NEXTVAL, ide, W_PERSONA);
  END IF;
...
W_ASIG                  ASIGNATURA.NOMBRE%TYPE)
AS 
  oida INTEGER;
  existe INTEGER;
BEGIN
  SELECT OID_A INTO oida FROM ASIGNATURA WHERE NOMBRE = W_ASIG;
  SELECT COUNT(*) INTO existe FROM POSEE
  WHERE (OID_H = W_OID_H AND OID_A = oida);
  IF (existe = 0) THEN
    INSERT INTO POSEE VALUES (SEC_POSEE.NEXTVAL,W_OID_H,oida);
    COMMIT WORK;
  END IF;
END INSERTAR_POSEE;
...
AS 
  ...
  existe INTEGER;
BEGIN
  SELECT OID_A INTO oida FROM ASIGNATURA WHERE W_ASIG = ASIGNATURA.NOMBRE;
  SELECT COUNT(*) INTO existe FROM REALIZA
  WHERE (W_OID_P = REALIZA.ALUMNO AND oida = REALIZA.ASIGNATURA);
  IF (existe = 0) THEN
    INSERT INTO REALIZA VALUES (SEC_REALIZA.NEXTVAL,W_OID_P,oida);
    COMMIT WORK;
  END IF;
...
W_ALUMNO            SE_RESPONSABILIZA_DE.ALUMNO%TYPE
  ...
  existe INTEGER;
BEGIN
  SELECT COUNT(*) INTO existe FROM SE_RESPONSABILIZA_DE
  WHERE (TUTOR = W_TUTOR AND ALUMNO = W_ALUMNO);
  IF (existe = 0) THEN
...
IF(W_CURSO = 'MUSICA Y MOVIMIENTO 1')
  THEN INSERT INTO MATRICULA VALUES(SEC_MATRICULA.NEXTVAL,W_AÑO,W_CURSO,30,W_ALUMNO);
      INSERTAR_PROFESOR('58566605A','Virginia Pérez','01/03/1987','685811394','vir_2004@hotmail.com');
      INSERTAR_ASIGNATURA('INICIACIÓN MUSICAL','58566605A',SEC_MATRICULA.CURRVAL, null);
      INSERTAR_HORARIO('LUNES','01/01/2000 16:00:00',2);
      INSERTAR_POSEE(SEC_HORARIO.CURRVAL,'INICIACIÓN MUSICAL');
      INSERTAR_REALIZA(W_ALUMNO, 'INICIACIÓN MUSICAL');
  COMMIT WORK;
...
ELSIF(W_CURSO = 'MUSICA Y MOVIMIENTO 3')
  THEN INSERT INTO MATRICULA VALUES(SEC_MATRICULA.NEXTVAL,W_AÑO,W_CURSO,35,W_ALUMNO);
   ...
   INSERTAR_PROFESOR('58566605A','Virginia Pérez','01/03/1987','685811394','vir_2004@hotmail.com');
   INSERTAR_ASIGNATURA('INICIACIÓN MUSICAL','58566605A',SEC_MATRICULA.CURRVAL, SEC_ESPECIALIDAD.CURRVAL);
...
ELSIF(W_CURSO = 'PREPARATORIO 1')
  ...
  END IF;
  INSERTAR_PROFESOR('58566605A','Virginia Pérez','01/03/1987','685811394','vir_2004@hotmail.com');
  INSERTAR_ASIGNATURA('LENGUAJE MUSICAL','58566605A',SEC_MATRICULA.CURRVAL, SEC_ESPECIALIDAD.CURRVAL);
  INSERTAR_HORARIO('LUNES','01/01/2000 17:00:00',2);
  INSERTAR_POSEE(SEC_HORARIO.CURRVAL,'LENGUAJE MUSICAL');
  INSERTAR_REALIZA(W_ALUMNO,'LENGUAJE MUSICAL');
  
  INSERTAR_PROFESOR('00848456D','Benito Vega','01/03/1987','685811394','benito@gmail.com');
  INSERTAR_ASIGNATURA('CORO','00848456D',SEC_MATRICULA.CURRVAL, SEC_ESPECIALIDAD.CURRVAL);
  INSERTAR_HORARIO('MARTES','01/01/2000 17:00:00',1);
  INSERTAR_POSEE(SEC_HORARIO.CURRVAL,'CORO');
  INSERTAR_REALIZA(W_ALUMNO, 'CORO');
   
  INSERTAR_PROFESOR('72162835J','Amparo Vega Cabrera','01/03/1987','685811394','vegamparo@hotmail.com');
  INSERTAR_ASIGNATURA('INSTRUMENTO COLECTIVO','72162835J',SEC_MATRICULA.CURRVAL, SEC_ESPECIALIDAD.CURRVAL);
  INSERTAR_HORARIO('MIÉRCOLES','01/01/2000 17:00:00',1);
  INSERTAR_POSEE(SEC_HORARIO.CURRVAL,'INSTRUMENTO COLECTIVO');
  INSERTAR_REALIZA(W_ALUMNO,'INSTRUMENTO COLECTIVO');
  COMMIT WORK;
...
ELSIF(W_CURSO = 'PRIMERO ELEMENTAL')
  ...
  INSERTAR_PROFESOR('72162835J','Amparo Vega Cabrera','01/03/1987','685811394','vegamparo@hotmail.com');
  INSERTAR_ASIGNATURA('INSTRUMENTO INDIVIDUAL','72162835J',SEC_MATRICULA.CURRVAL, SEC_ESPECIALIDAD.CURRVAL);
  INSERTAR_HORARIO('MIÉRCOLES','01/01/2000 17:00:00',1);
  INSERTAR_POSEE(SEC_HORARIO.CURRVAL,'INSTRUMENTO INDIVIDUAL');
  INSERTAR_REALIZA(W_ALUMNO,'INSTRUMENTO INDIVIDUAL');
...
ELSIF(W_CURSO = 'SEGUNDO ELEMENTAL')
  ...
  INSERTAR_PROFESOR('58566605A','Virginia Pérez','01/03/1987','685811394','vir_2004@hotmail.com');
  INSERTAR_ASIGNATURA('BANDA','58566605A',SEC_MATRICULA.CURRVAL, SEC_ESPECIALIDAD.CURRVAL);
  INSERTAR_HORARIO('VIERNES','01/01/2000 20:00:00',1);
  INSERTAR_POSEE(SEC_HORARIO.CURRVAL,'BANDA');
  INSERTAR_REALIZA(W_ALUMNO,'BANDA');
  COMMIT WORK;
 ELSIF(W_CURSO = 'TERCERO ELEMENTAL')
  ...
  if(numIn >  1)
   THEN UPDATE MATRICULA SET PRECIO=60 WHERE oid_M=seq;
  END IF;
  INSERTAR_PROFESOR('92479300A','Cristina Guzmán','01/03/1987','685811394','vir_2004@hotmail.com');
  INSERTAR_ASIGNATURA('LENGUAJE MUSICAL','92479300A',SEC_MATRICULA.CURRVAL, SEC_ESPECIALIDAD.CURRVAL);
  INSERTAR_HORARIO('LUNES','01/01/2000 17:00:00',2);
  INSERTAR_POSEE(SEC_HORARIO.CURRVAL,'LENGUAJE MUSICAL');
  INSERTAR_REALIZA(W_ALUMNO,'LENGUAJE MUSICAL');
  ...
  INSERTAR_PROFESOR('06328209N','Carlos Fernández','01/03/1987','685811394','carlos@hotmail.com');
  INSERTAR_ASIGNATURA('BANDA','06328209N',SEC_MATRICULA.CURRVAL, SEC_ESPECIALIDAD.CURRVAL);
  INSERTAR_HORARIO('VIERNES','01/01/2000 20:00:00',1);
  INSERTAR_POSEE(SEC_HORARIO.CURRVAL,'BANDA');
  INSERTAR_REALIZA(W_ALUMNO,'BANDA');
  COMMIT WORK;
ELSIF(W_CURSO = 'CUARTO ELEMENTAL')
  ...
  INSERTAR_PROFESOR('00848456D','Benito Vega','01/03/1987','685811394','vir_2004@gmail.com');
  INSERTAR_ASIGNATURA('CORO','00848456D',SEC_MATRICULA.CURRVAL, SEC_ESPECIALIDAD.CURRVAL);
  INSERTAR_HORARIO('MARTES','01/01/2000 17:00:00',1);
  INSERTAR_POSEE(SEC_HORARIO.CURRVAL,'CORO');
  INSERTAR_REALIZA(W_ALUMNO, 'CORO');
...
ELSIF(W_CURSO = 'SOLO INSTRUMENTO')
  ...
  INSERTAR_PROFESOR('06328209N','Carlos Fernández','01/03/1987','685811394','carlos@hotmail.com');
  INSERTAR_ASIGNATURA('INSTRUMENTO INDIVIDUAL','06328209N',SEC_MATRICULA.CURRVAL, SEC_ESPECIALIDAD.CURRVAL);
  INSERTAR_HORARIO('VIERNES','01/01/2000 20:00:00',1);
  INSERTAR_POSEE(SEC_HORARIO.CURRVAL,'INSTRUMENTO INDIVIDUAL');
  INSERTAR_REALIZA(W_ALUMNO,'INSTRUMENTO INDIVIDUAL');
  COMMIT WORK;
...
BEGIN
  ...
  IF((edad>5 AND edad<8) AND NOT(:NEW.CURSO LIKE 'PREPARATORIO%'))
    THEN
